...
export async function updateUserSettings(userId, updates) {
  try {
    console.log("Updating user settings for user ID:", userId);
    console.log("Updates to be applied:", updates);

    const validUpdates = {};
    if ('username' in updates) validUpdates.username = updates.username;
    if ('email' in updates) validUpdates.email = updates.email;
    if ('avatar' in updates) validUpdates.avatar = updates.avatar;
    if ('notifications' in updates) validUpdates.notifications = updates.notifications;
    if ('gpaVisibility' in updates) validUpdates.gpaVisibility = updates.gpaVisibility;
    if ('gpa' in updates && updates.gpa !== '') {
      validUpdates.gpa = parseFloat(updates.gpa);
      if (isNaN(validUpdates.gpa)) {
        throw new Error("Invalid GPA value");
      }
    }

    console.log("Valid updates to be sent:", validUpdates);
